...
public class MatchCards {
    class Card {  // to represent the cards it is a simple class
        //data members
        String cardName;
        ImageIcon cardImageIcon;// as we are pressing on the card it is a button so we are setting an imageicon for all cards

        Card(String cardName, ImageIcon cardImageIcon) { // constructor to input the name of the card and the card image
            this.cardName = cardName;
            this.cardImageIcon = cardImageIcon;
        }

        public String toString() {//when you want to print out a class you need a string representation so we are using the string
            return cardName;
        }
    }
    ...
    ImageIcon cardBackImageIcon; // used for back image of the card

    int boardWidth = columns * cardWidth; // 5*128 = 640px
    int boardHeight = rows * cardHeight; // 4*90 = 360px

    JFrame frame = new JFrame("Match Cards"); // Jframe is used to enter the title of the frame
    JLabel textLabel = new JLabel(); // JLabel is used to diaplay a single line in read only text
    JPanel textPanel = new JPanel(); // JPanel is used to
    ...
    MatchCards() {
        ...
        // error text
        textLabel.setFont(new Font("Jazz", Font.PLAIN, 20));
